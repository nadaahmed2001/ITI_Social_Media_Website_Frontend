    ...
    addComment(post.id, commentData)
      .then((res) => {
        ...
        if (!newComment.author_id && user) {
          ...
          newComment.author_profile_picture = user?.profile_picture || currentUserAvatar;
        }
        ...
        <div className="flex items-center space-x-3">
          <Link to={`/profiles/${post.author_id}`} className="group flex-shrink-0 hover:opacity-90 transition-opacity">
            <img
              ...
              className="w-11 h-11 rounded-full object-cover border-2 border-[#7a2226]/30 group-hover:border-[#7a2226]/50 transition-all shadow-sm"
              onError={(e) => {
          ...
          <div>
            <Link to={`/profiles/${post.author_id}`} className="hover:opacity-90 transition-opacity !no-underline">
              <p className="mb-0 font-semibold text-base bg-gradient-to-r from-[#7a2226] to-[#a53d3d] bg-clip-text text-transparent">
                {avatarTitleText || "Unknown"}
              </p>
            </Link>
        ...
        <div className="post-actions flex justify-around items-center border-y border-gray-200/80 px-1 py-1.5 mt-1">
          ...
          <div className="relative flex-1 mx-1" onMouseEnter={handleMouseEnterTrigger} onMouseLeave={handleMouseLeaveArea}>
            ...
            <button
              ...
              className={`w-full flex justify-center items-center gap-1.5 py-1.5 rounded-md text-sm font-medium transition-colors duration-200 hover:bg-[#7a2226]/10 ${currentUserReactionType ? getReactionColorClass(currentUserReactionType) + ' font-semibold' : 'text-gray-600 hover:text-[#7a2226]'
                }`}
            >
