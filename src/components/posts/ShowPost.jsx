...
//                       className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-1 flex space-x-1 bg-white shadow-lg rounded-full px-3 py-1.5 border border-gray-200 z-20" // White background, adjusted padding
//                   >
//                       {/* Map through AVAILABLE_REACTIONS_DATA */}
//                       {AVAILABLE_REACTIONS_DATA.map((reaction) => {
//                           // *** FIX: Apply text color class to the icon ***
//                           const colorClass = getReactionColorClass(reaction.name);
//                           return (
//                               <button
//                                   key={reaction.name}
//                                   onClick={(e) => { e.stopPropagation(); handleAddReaction(reaction.name); setShowReactions(false); }}
//                                   // Styling for each reaction icon button
...
//                                   title={reaction.name}
//                               >
//                                   {/* Render the icon component directly, applying color */}
//                                   {React.cloneElement(reaction.icon, { className: `w-6 h-6 ${colorClass}` })}
//                               </button>
//                           );
//                        })}
//                   </div>
...
const handleRemoveReaction = async () => {
  ...
  function SampleNextArrow(props) {
      const { className, style, onClick } = props;
      const isDisabled = className?.includes('slick-disabled');
      return ( <button type="button" className={`absolute top-1/2 right-2 md:right-4 -translate-y-1/2 z-10 p-3 bg-black bg-opacity-30 rounded-full text-white hover:bg-opacity-50 transition-opacity duration-200 flex items-center justify-center ${isDisabled ? 'opacity-40 cursor-not-allowed' : 'cursor-pointer'} ${className}` } style={{ ...style }} onClick={onClick} aria-label="Next attachment" disabled={isDisabled} > <ArrowForwardIos fontSize="large" /> </button> );
  }
  function SamplePrevArrow(props) {
      const { className, style, onClick } = props;
      const isDisabled = className?.includes('slick-disabled');
      return ( <button type="button" className={`absolute top-1/2 left-2 md:left-4 -translate-y-1/2 z-10 p-3 bg-black bg-opacity-30 rounded-full text-white hover:bg-opacity-50 transition-opacity duration-200 flex items-center justify-center ${isDisabled ? 'opacity-40 cursor-not-allowed' : 'cursor-pointer'} ${className}` } style={{ ...style }} onClick={onClick} aria-label="Previous attachment" disabled={isDisabled} > <ArrowBackIosNew fontSize="large" /> </button> );
  }
  ...
  return (
    ...
    <div key={post.id} className="bg-white/95 backdrop-blur-sm rounded-xl shadow-lg p-6 mb-6 border border-gray-200/80 transition-all duration-300 hover:shadow-xl"> {/* Adjusted styles */}
      ...
      {numAttachments > 0 && (
          <div className="mt-3 slick-slider-container relative border border-gray-200/80 rounded-lg overflow-hidden"> {/* Adjusted border */}
            <Slider {...sliderSettings}>
              {/* Map over ALL attachments */}
                {attachments.map((attachment, index) => (
                  // Each child of Slider is a slide
